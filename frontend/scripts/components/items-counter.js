const incrementDecrementValue = function() {
  ...
  const input = $(this).next().hasClass('input-field_underlined') ? $(this).next() : $(this).prev();
  const operation = $(this).next().hasClass('input-field_underlined') ? 'decrement' : 'increment';
